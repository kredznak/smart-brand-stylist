...
Notes: logo color extraction runs entirely in the browser. AI requests go to our Cloudflare Worker, which calls Anthropic; the server keeps no user data and applies per-user rate limits (10 per minute, 60 per day). Privacy policy, terms and help are linked in the listing.
